<?php 
	...
	function displayTweets($type) {
        ...
        if ($type == 'public') {
			...
			echo "<p>Showing search results for <strong>".mysqli_real_escape_string($link, $_GET['q'])."</strong>:</p>";
		...
		if (mysqli_num_rows($result) == 0) {
			...
			while ($row = mysqli_fetch_assoc($result)) {
				...
				echo "<div class='tweet'>
						<p><a href='?page=publicprofiles&userid=".$user['id']."'>".$user['email']."</a>  <span class='time'>".time_since(time() - strtotime($row['datetime']))." ago</span> :</p> 
						<p>".$row['tweet']."</p>
						<p><a href='#' class='toggleFollow' data-userId='".$row['userid']."'>";
	...
	function displaySearch() {
		...
		echo '<form class="form-inline form-group">
    ...
    function displayUsers() {
		...
		while ($row = mysqli_fetch_assoc($result)) {
			...
			echo "<p><a href='?page=publicprofiles&userid=".$row['email']."'>".$row['email']."</a></p>";
